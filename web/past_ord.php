...
$uid=$_SESSION['u_id'];
$wx=0;
$cart_sql="SELECT * FROM `cart` WHERE `cart_status` = 2 OR `cart_status` = 3 AND `cart_uid` = ".$uid;
$cart_query=mysqli_query($con,$cart_sql);
$cart_count = mysqli_num_rows($cart_query);
$ords_sql="SELECT * FROM `cart` WHERE `cart_uid`= ".$uid." AND `cart_status` = 2 OR `cart_status` = 3 ORDER BY `cart_id` DESC";
