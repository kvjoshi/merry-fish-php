...
$wx=0;
$cart_sql="SELECT * FROM `cart` WHERE `cart_status` = 1";
$cart_query=mysqli_query($con,$cart_sql);
$cart_count = mysqli_num_rows($cart_query);
$ords_sql="SELECT * FROM `cart`,`distributor` where `distributor`.`d_id`=`cart`.`cart_uid` and `cart`.`cart_status` = 1 ORDER BY `cart_id` DESC ";
                        ...
                        <div class="portlet-box portlet-gutter  mb-10 row">
                            <div class="portlet-header flex-row flex d-flex align-items-center b-b">
                                <div class="flex d-flex flex-column">
                                    <h3>Total Orders Pending Approval</h3>
                                </div>
                            </div>
